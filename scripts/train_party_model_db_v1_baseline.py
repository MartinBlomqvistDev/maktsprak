...
# Reference batch/LR for OneCycleLR max_lr scaling (identity at the default batch).
BASE_BATCH_SIZE = TRAIN_BATCH_SIZE
BASE_MAX_LR = TRAIN_MAX_LR
...
def get_training_data():
    # --- 1. Speeches from Supabase (Safe Year-by-Year Chunking) ---
    years = range(2015, 2027)  # Fetches years 2015 through 2026
    speech_dfs = []
    
    for year in years:
        print(f"Safe DB Query: Fetching year {year}...")
        try:
            # Try passing both start and end date if your function supports it
            year_df = fetch_speeches_historical(start_date=f"{year}-01-01", end_date=f"{year}-12-31")
        except TypeError:
            # Fallback if fetch_speeches_historical natively only accepts start_date
            year_df = fetch_speeches_historical(start_date=f"{year}-01-01")
            # Filter in-memory so the dataframe doesn't balloon with future years
            if "date" in year_df.columns:
                year_df = year_df[year_df["date"].astype(str).str.startswith(str(year))]
            
        print(f"  Successfully loaded {len(year_df)} rows.")
    ...
    # --- 2. Tweets from Supabase (Your Original Logic) ---
    ...
    # --- 3. Combine and Filter (Your Original Logic) ---
...
if __name__ == "__main__":
    ...
    _p = argparse.ArgumentParser(description="Train the party classifier from the DB.")
    _p.add_argument(
        "--no-fgm",
        action="store_true",
        help="Disable FGM adversarial training (~2x faster; use for a cheap baseline).",
    )
    _p.add_argument("--max-length", type=int, default=MAX_LENGTH, help=f"Token length (default {MAX_LENGTH}).")
    _p.add_argument("--epochs", type=int, default=MAX_EPOCHS, help=f"Max epochs (default {MAX_EPOCHS}).")
    _p.add_argument("--batch-size", type=int, default=BATCH_SIZE, help=f"Batch size (default {BATCH_SIZE}).")
    _args = _p.parse_args()

    # Runtime overrides (fast/cheap pass vs full adversarial run).
    USE_FGM = not _args.no_fgm
    MAX_LENGTH = _args.max_length
    MAX_EPOCHS = _args.epochs
    BATCH_SIZE = _args.batch_size
    scaled_max_lr = BASE_MAX_LR * (BATCH_SIZE / BASE_BATCH_SIZE)

    print("Startar träning...")
    print(
        f"Inställningar: FGM={USE_FGM} | MAX_LENGTH={MAX_LENGTH} | "
    ...
    for epoch in range(start_epoch, MAX_EPOCHS):
        ...
        for batch in loop:
            ...
            # FGM adversarial step (optional; skipped with --no-fgm)
            if USE_FGM:
                fgm.attack()
                with torch.cuda.amp.autocast(enabled=(DEVICE.type=="cuda")):
