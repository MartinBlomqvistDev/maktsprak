...
**Is a fine-tuned model worth it against a frontier LLM?** ([`llm_classify_benchmark.py`](research/llm_classify_benchmark.py)). The deployed 110M classifier against six frontier LLMs on party classification, over the model's own speaker-independent held-out set (2015-2026 speeches by speakers held out of training, verified clean: 0.59 held-out vs 0.99 on training speakers), with real per-token cost measured. The comparison is accuracy against cost, latency, and whether the text has to leave the machine at all. Being run on the deployed model; the page is a placeholder until the numbers are honest.
